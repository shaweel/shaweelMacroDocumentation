...
[jar](#jar-file)
## Debian
For [Debian](https://www.debian.org) and its [derivatives](https://www.debian.org/derivatives/) you can install shaweelMacro by downloading a .deb file from the [latest release](github.com/shaweel/shaweelMacro/releases/latest) and installing it with `sudo apt install ./shaweelMacro.deb`

## Fedora
For [Fedora](https://fedoraproject.org) and its [derivatives](https://fedoraproject.org/wiki/Derived_distributions) you can install shaweelMacro by downloading a .rpm file from the [latest release](github.com/shaweel/shaweelMacro/releases/latest) and installing it with `sudo dnf install ./shaweelMacro.rpm`
...
To install shaweelMacro via `flatpak` download a .flatpak file from the [latest release](github.com/shaweel/shaweelMacro/releases/latest) and install it with `flatpak install ./shaweelMacro.flatpak`
...
After making sure you have the Java 25 JRE or JDK installed, you can download a .jar file from the [latest release](github.com/shaweel/shaweelMacro/releases/latest) and simply run it with `java -jar ./shaweelMacro.jar` or on some [Desktop Environments](https://en.wikipedia.org/wiki/Desktop_environment) just opening the file.